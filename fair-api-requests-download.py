...
def parse_arguments():
    ...
    parser.add_argument('-d', '--detailed', action='store_true',
                        help='download all requests as separate CSVs', required=False)
...
def get_requests():
    ...
    r = requests.get(requests_list_endpoint,
                     headers=AUTHENTICATED_HEADERS, verify=SSL_VERIFY)
    requests_array = []

    if r.status_code != 200:
        error_data = r.json()
        print(
            f'Failed to retrieve request list. Status code: {r.status_code}, Error message: {error_data["error"]["message"]}')
        return False
    data = r.json()
    for req in data["items"]:
        ...
        if r.status_code != 200:
            try:
                error_data = r.json()
                print(
                    f'Failed to retrieve request. Status code: {r.status_code}, Error message: {error_data["error"]["message"]}')
            except Exception:
                print(
                    f'Failed to retrieve request. Status code: {r.status_code}')
        else:
            request_data = r.json()
